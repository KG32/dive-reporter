use std::{error::Error, fs, path::PathBuf, fmt::format, os::macos::raw::stat};
use crate::parser::{self, UDDFDoc, DiveElem};
...
impl Stats {
    pub fn from_file(path: &str) -> Result<Stats, Box<dyn Error>> {
        ...
        Ok(stats)
        // --
        // let dive_stats = Self::handle_file(path);
        // dbg!(dive_stats);
        // Ok(stats)
    }
